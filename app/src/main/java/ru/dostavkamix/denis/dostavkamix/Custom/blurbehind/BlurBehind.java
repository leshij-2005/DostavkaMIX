package ru.dostavkamix.denis.dostavkamix.Custom.blurbehind;
...
import ru.dostavkamix.denis.dostavkamix.Custom.blurbehind.util.Blur;

public class BlurBehind {

    private static final String KEY_CACHE_BLURRED_BACKGROUND_IMAGE = "KEY_CACHE_BLURRED_BACKGROUND_IMAGE";
    private static final int CONSTANT_BLUR_RADIUS = 2;
    ...
    public void execute(Activity activity, OnBlurCompleteListener onBlurCompleteListener, int radius) {
        if (mState.equals(State.READY)) {
            mState = State.EXECUTING;
            cacheBlurBehindAndExecuteTask = new CacheBlurBehindAndExecuteTask(activity, onBlurCompleteListener, radius);
    ...
    private class CacheBlurBehindAndExecuteTask extends AsyncTask<Void, Void, Void> {
        ...
        public CacheBlurBehindAndExecuteTask(Activity activity, OnBlurCompleteListener onBlurCompleteListener, int radius) {
        ...
        protected Void doInBackground(Void... params) {
            Bitmap blurredBitmap = Blur.apply(activity, image, radius);
